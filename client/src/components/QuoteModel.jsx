...
const QuoteModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const backendUrl = 'http://localhost:5000/send-quote-request'; // EXAMPLE URL
    ...
    try {
      ...
      if (response.ok && result.success) {
        setStatus('success');
        setFormData({ name: '', company: '', email: '', phone: '', city: '', message: '' }); // Clear form
        // Optional: Close modal automatically after a delay
        // setTimeout(() => {
        //   onClose(); // Call the onClose prop passed from the parent
        //   setStatus('idle'); // Reset status if modal is closed
        // }, 2000); // Close after 2 seconds
      } else {
        setStatus('error');
        setErrorMessage(result.error || 'An unknown error occurred processing your request.');
        console.error("Backend Error:", result);
      }
    } catch (error) {
      console.error("Network/Fetch Error:", error);
      setStatus('error');
      setErrorMessage('Could not connect to the quote server. Please try again later.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      {/* Modal Content */}
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 my-8 overflow-hidden"> {/* Increased max-w */}
        {/* Modal Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
          <h2 className="text-xl font-semibold text-[#003229]"> {/* Adjusted style */}
        ...
        <form onSubmit={handleSubmit}> {/* Add onSubmit handler to form */}
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto"> {/* Add scroll for smaller screens */}

              {/* Name */}
              <div>
                <label htmlFor="quote-name" className="block text-sm font-medium text-gray-700 mb-1"> {/* Added margin */}
                  Name <span className="text-red-500">*</span> {/* Indicate required */}
                </label>
                <input
                  type="text"
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
                ...
                className={`w-full bg-[#003229] text-white py-2.5 px-4 rounded-lg hover:bg-[#014739] transition text-base font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#003229] ${
                    (status === 'sending' || status === 'success') ? 'opacity-60 cursor-not-allowed' : '' // Style when disabled
                }`}
                >
                {status === 'sending' ? 'Sending Request...' : status === 'success' ? 'Sent!' : 'Submit Quote Request'}
                </button>
            </div>
        </form> {/* End of form */}
